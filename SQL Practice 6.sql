...
# Using group by for finding the sum for finding students marks for a particular window or group 
select student_batch, sum(students_marks) from ineuron_students group by student_batch;

# Using group by for finding the min for finding students marks for a particular window or group 
select student_batch, min(students_marks) from ineuron_students group by student_batch;

# Using group by for finding the max for finding students marks for a particular window or group 
select student_batch, max(students_marks) from ineuron_students group by student_batch;

# Using group by for finding the avg for finding students marks for a particular window or group 
select student_batch, avg(students_marks) from ineuron_students group by student_batch;

# Using group by to count number of values in student batch by using group by command 
...
# Selecting the person scoring highest marks by using sub query 
...
# Now for looking at the second heighest marks with its name we use 
select * from ineuron_students where student_batch = 'fsda' order by students_marks desc limit 1,1;

# Another approch is window function syntax which can be used for a big record 
select student_id, student_batch, student_strem, students_marks , row_number() over(partition by student_batch order by students_marks) as 'row_number' from ineuron_students;

select * from (select student_id , student_batch , student_strem,students_marks ,
...
select student_id,  student_batch, student_strem, students_marks,
...
# In the above window function command we are not getting a unique rank and in a proper sequence so we use method as shown below to solve this problem 
# dense rank 

select student_id, student_batch, student_strem, students_marks, 
row_number() over(order by students_marks desc) as 'row_numer',
rank() over(order by students_marks desc) as 'row_rank',
